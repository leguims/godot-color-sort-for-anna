...
func initialiser_un_nouveau_niveau(nb_niveau : int, # TODO : INUTILISE !
										niveau_debut : int,
										niveau_fin : int) -> bool:
	"Crée et initialise une Nouveau niveau"
...
func lire_niveau_debut_niveau() -> int:
	var enregistrement_campagne = lire_dernier_niveau()
	if enregistrement_campagne:
		return enregistrement_campagne.get('niveau_debut')
	return 0

func lire_niveau_fin_niveau() -> int:
	var enregistrement_campagne = lire_dernier_niveau()
	if enregistrement_campagne:
...
func lire_pourcentage_niveau_realise() -> int:
	...
	if niveau_en_cours():
		var nb_niveaux_totaux = lire_niveau_longueur_initiale()
...
func incrementer_longueur_detour_niveau() -> void:
	var enregistrement_campagne = lire_dernier_niveau()
	if enregistrement_campagne:
...
func lire_ratio_reussite_niveau() -> int:
	"Pourcentage de réussite de Le niveau (retourne 99 pour 99%, 15 pour 15% ...)"
	if niveau_existe():
		var nb_essais  = _lire_nombre_plateaux()
		var nb_detours  = lire_longueur_detour_niveau()
...
func abandonner_un_plateau() -> void:
	...
	incrementer_longueur_detour_niveau()
